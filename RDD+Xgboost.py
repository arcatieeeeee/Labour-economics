...
def run_xgb_analysis(df):
    results = {}

    for work_type in ["Full-time", "Part-time"]:
        print(f"\n🚀 训练 XGBoost RDD 模型: {work_type}")
        ...
        plt.title(
            f"RDD Analysis with Policy Effect: {work_type}\nEffect: {policy_result['effect_size']:.2f} ± {policy_result['std_dev']:.2f}")
        plt.xlabel("Log Months Since Policy")
